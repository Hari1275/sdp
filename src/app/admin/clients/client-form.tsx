...
export function ClientForm() {
  ...
  const filteredAreas = safeAreas.filter(
    (area) => area.regionId === selectedRegionId
  );
  const filteredMRs = safeUsers.filter(
    (user) => user.regionId === selectedRegionId
  );
  ...
  const fetchFormData = async () => {
    ...
    try {
      ...
      console.log(
        "[ClientForm] Regions response status:",
        regionsResponse.status
      );
      ...
      if (regionsResponse.ok) {
        ...
        if (regionsResult.success) {
          console.log(
            "[ClientForm] Regions API success, data:",
            regionsResult.data
          );
          const regionData = Array.isArray(regionsResult.data)
            ? regionsResult.data
            : [];
          setRegions(regionData);
          console.log(
            "[ClientForm] Set regions array with",
            regionData.length,
            "items:",
            regionData
          );
          ...
          console.error(
            "[ClientForm] Regions API returned success:false:",
            regionsResult
          );
          setRegions([]);
        }
        ...
        console.error(
          "[ClientForm] Failed to fetch regions:",
          regionsResponse.status,
          errorText
        );
        setRegions([]);
      ...
      if (areasResponse.ok) {
        ...
        const areaData = Array.isArray(areasResult.data)
          ? areasResult.data
          : [];
        setAreas(areaData);
        ...
        console.error(
          "[ClientForm] Failed to fetch areas:",
          areasResponse.status
        );
        setAreas([]);
      }
      ...
      if (usersResponse.ok) {
        ...
        console.error(
          "[ClientForm] Failed to fetch users:",
    ...
    if (isSheetOpen) {
      if (selectedClient) {
        form.reset({
          ...
          businessType: selectedClient.businessType as
            | "CLINIC"
            | "HOSPITAL"
            | "PHARMACY"
            | "MEDICAL_STORE"
  ...
  useEffect(() => {
    if (selectedRegionId) {
      ...
      if (
        currentAreaId &&
        !filteredAreas.find((area) => area.id === currentAreaId)
      ) {
  ...
  const onSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      console.log("[ClientForm] Submitting data:", {
        ...
        isValidObjectId: {
          region:
            data.regionId.length === 24 && /^[0-9a-fA-F]+$/.test(data.regionId),
                              ...
                              <SelectContent>
                                {isLoadingData ? (
                                  ...
                                  <SelectItem
                                    ...
                                    disabled
                                  >
                                    No regions available (API returned{" "}
                                    {safeRegions.length} items)
                                  </SelectItem>
                                )}
                              </SelectContent>
                            </Select>
                            ...
                            >
                              ...
                              <SelectContent>
                                {isLoadingData ? (
                                  ...
                                  <div className="p-2 text-sm text-muted-foreground">
                                    No MRs available for this region
                                  </div>
                                )}
                              </SelectContent>
                            </Select>
