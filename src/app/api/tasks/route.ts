  ...
  try {
    ...
    if (regionId) {
      ...
      if (user.role === UserRole.LEAD_MR && regionId !== user.regionId) {
        return errorResponse('FORBIDDEN', 'You can only access tasks in your region', 403);
      }
      if (user.role === UserRole.MR) {
        return errorResponse('FORBIDDEN', 'MR users cannot filter by region', 403);
      }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!hasPermission(user.role, [UserRole.ADMIN, UserRole.LEAD_MR])) {
      return errorResponse('FORBIDDEN', 'Only Lead MR and Admin can create tasks', 403);
    }
    ...
    if (user.role === UserRole.LEAD_MR && taskData.regionId !== user.regionId) {
      return errorResponse('FORBIDDEN', 'You can only create tasks in your region', 403);
    }
    ...
    if (taskData.areaId) {
      ...
      if (area.regionId !== taskData.regionId) {
        return errorResponse('VALIDATION_ERROR', 'Area must belong to the specified region');
      }
    }
