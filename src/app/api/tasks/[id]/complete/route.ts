  ...
  try {
    ...
    try {
      ...
      if (body.trim()) {
        ...
        // Completion data validated - can be extended in future for completion notes
      }
    } catch {
    ...
    if (existingTask.status === TaskStatus.COMPLETED) {
      return errorResponse('TASK_ALREADY_COMPLETED', 'Task is already completed', 400);
    }
    ...
    if (!canComplete) {
      return errorResponse('FORBIDDEN', 'You are not authorized to complete this task', 403);
    }
    ...
    console.log(`Task completed: ${completedTask.id} by user: ${user.id} (${user.name}) at ${new Date().toISOString()}`);
