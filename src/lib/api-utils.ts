...
export function rateLimit(request: NextRequest, limit = 100, windowMs = 15 * 60 * 1000): boolean {
...
export async function getAuthenticatedUser(request: NextRequest) {
  try {
    ...
    const { extractTokenFromRequest, getAuthenticatedUserFromToken, JWTError } = await import('./jwt')
...
export function withAuth(
  handler: (request: NextRequest, user: AuthenticatedUser, context?: Record<string, unknown>) => Promise<NextResponse>,
...
export function validateRequest<T>(
  ...
  try {
    ...
    const errorMessage = (error as { issues?: Array<{ message: string }> })?.issues?.map(issue => issue.message).join(', ') || 'Validation failed'
...
export function applyRoleBasedFilters(user: AuthenticatedUser, baseWhere: FilterWhere = {}): FilterWhere {
...
// Error logging
export function logError(error: Error | unknown, context: string, userId?: string) {
